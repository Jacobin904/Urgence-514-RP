...
const RESULTS_CHANNEL_ID = process.env.RESULTS_CHANNEL_ID || '1475659638618980515';
...
// ===== BASE DE DONNÉES (GitHub = persistant, survit aux redéploiements) =====
...
async function saveApplications(list){
  if (GITHUB_TOKEN){
    for (let attempt = 0; attempt < 3; attempt++){
      ...
      if (r.ok) return;
      if (r.status === 409 || r.status === 422) continue; // conflit : réessaie
...
// ===== BOT DISCORD (en ligne sur Render) =====
const bot = new Client({ intents: [GatewayIntentBits.Guilds] });
bot.once('ready', () => {
  console.log('Bot en ligne : ' + bot.user.tag);
...
if (BOT_TOKEN){
  bot.login(BOT_TOKEN).catch(e => console.error('Bot login error:', e.message));
}

// Vérifie les rôles côté serveur avec le token du bot (sécurisé)
...
// Le bot envoie les embeds lui-même (privé : personne + résultat seulement)
async function sendEmbed(channelId, embed){
  ...
  }catch(e){ console.error('sendEmbed error:', e.message); }
}
...
app.post('/api/applications', async (req, res) => {
  ...
  // Notification : seulement la personne (réponses privées)
  await sendEmbed(RESULTS_CHANNEL_ID, {
...
app.post('/api/applications/:discordId/:action', requireAdmin, async (req, res) => {
  ...
  // Résultat : personne + décision seulement (réponses privées)
  await sendEmbed(RESULTS_CHANNEL_ID, {
