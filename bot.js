...
addCmd(new SlashCommandBuilder().setName('avatar').addUserOption(option => option.setName('utilisateur').setDescription('L\'utilisateur (optionnel)')), async i => { const u = i.options.getUser('utilisateur') || i.user; i.reply({ embeds: [baseEmbed().setTitle(`Avatar de ${u.username}`).setImage(u.displayAvatarURL({ size: 512 }))] }); });
addCmd(new SlashCommandBuilder().setName('userinfo').addUserOption(option => option.setName('utilisateur').setDescription('L\'utilisateur (optionnel)')), async i => { const u = i.options.getUser('utilisateur') || i.user; const m = await i.guild.members.fetch(u.id).catch(() => null); i.reply({ embeds: [baseEmbed().setTitle(`👤 ${u.username}`).addFields({name:'ID',value:u.id,inline:true},{name:'Bot ?',value:u.bot?'Oui':'Non',inline:true},{name:'Compte créé',value:`<t:${Math.floor(u.createdTimestamp/1000)}:R>`,inline:false},{name:'A rejoint',value:m?`<t:${Math.floor(m.joinedTimestamp/1000)}:R>`:'?',inline:false},{name:'Rôles',value:m&&m.roles.cache.size>1?m.roles.cache.filter(r=>r.id!==i.guild.id).map(r=>r.toString()).join(' '):'Aucun',inline:false}).setThumbnail(u.displayAvatarURL({ size: 256 }))] }); });
...
// --- MODÉRATION (TOUTES LES OPTIONS ONT MAINTENANT UNE DESCRIPTION) ---
addCmd(new SlashCommandBuilder().setName('warn')
  ...
  .addStringOption(option => option.setName('raison').setDescription('La raison de l\'avertissement').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('warnings')
  .addUserOption(option => option.setName('utilisateur').setDescription('L\'utilisateur à vérifier').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('clearwarns')
  .addUserOption(option => option.setName('utilisateur').setDescription('L\'utilisateur dont il faut effacer les avertissements').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('kick')
  ...
  .addStringOption(option => option.setName('raison').setDescription('La raison du kick').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('ban')
  ...
  .addStringOption(option => option.setName('raison').setDescription('La raison du ban').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('unban')
  .addStringOption(option => option.setName('id').setDescription('L\'ID de l\'utilisateur à débannir').setRequired(true)), async i => {
  if (!i.member.permissions.has(PermissionFlagsBits.BanMembers)) return i.reply({ content: '❌ Permission insuffisante.', ephemeral: true });
...
addCmd(new SlashCommandBuilder().setName('timeout')
  ...
  .addStringOption(option => option.setName('raison').setDescription('La raison du timeout').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('clear')
  .addIntegerOption(option => option.setName('nombre').setDescription('Nombre de messages à supprimer (1-100)').setRequired(true).setMinValue(1).setMaxValue(100)), async i => {
...
addCmd(new SlashCommandBuilder().setName('slowmode')
  .addIntegerOption(option => option.setName('secondes').setDescription('Durée du slowmode en secondes (0-21600)').setRequired(true).setMinValue(0).setMaxValue(21600)), async i => { 
  if (!isStaff(i)) return i.reply({ content: '❌ Permission insuffisante.', ephemeral: true }); 
  await i.channel.setRateLimitPerUser(i.options.getInteger('secondes')); i.reply({ content: `🐢 Slowmode : ${i.options.getInteger('secondes')}s.` }); 
});
...
addCmd(new SlashCommandBuilder().setName('say')
  .addStringOption(option => option.setName('message').setDescription('Le message à envoyer').setRequired(true)), async i => { 
  if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true }); 
  await i.channel.send(i.options.getString('message')); i.reply({ content: '✅ Envoyé.', ephemeral: true }); 
});
...
addCmd(new SlashCommandBuilder().setName('embed')
  ...
  .addStringOption(option => option.setName('couleur').setDescription('Couleur en hex sans # (ex: 0B5BD7)')), async i => {
  if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true });
...
addCmd(new SlashCommandBuilder().setName('annonce')
  .addStringOption(option => option.setName('message').setDescription('Le message de l\'annonce').setRequired(true)), async i => { 
  if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true }); 
  await i.channel.send({ embeds: [baseEmbed().setTitle('📢 Annonce').setDescription(i.options.getString('message'))] }); i.reply({ content: '✅ Annonce publiée.', ephemeral: true }); 
});

addCmd(new SlashCommandBuilder().setName('giveaway')
  .addStringOption(option => option.setName('duree').setDescription('Durée (ex: 10m, 1h, 1d)').setRequired(true))
  .addStringOption(option => option.setName('prix').setDescription('Le prix du giveaway').setRequired(true)), async i => {
  if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true });
  const ms = parseDuration(i.options.getString('duree')); if (!ms) return i.reply({ content: 'Durée invalide.', ephemeral: true });
