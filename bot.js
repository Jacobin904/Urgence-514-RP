const { Client, GatewayIntentBits, EmbedBuilder, PermissionFlagsBits, SlashCommandBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, ActivityType } = require('discord.js');
...
addCmd(new SlashCommandBuilder().setName('regles').addStringOption(o => o.setName('type').setDescription('Quel règlement ?').setRequired(true).addChoices({name:'Discord',value:'discord'},{name:'Roblox',value:'roblox'})), async i => {
...
// ✅ CORRECTION : suppression de .setRequired(false) qui causait le bug de sérialisation
addCmd(new SlashCommandBuilder().setName('langage').addStringOption(o => o.setName('terme').setDescription('Terme à chercher (optionnel)')), async i => {
...
addCmd(new SlashCommandBuilder().setName('avatar').addUserOption(o => o.setName('utilisateur').setDescription('Utilisateur (optionnel)')), async i => { const u = i.options.getUser('utilisateur') || i.user; i.reply({ embeds: [baseEmbed().setTitle(`Avatar de ${u.username}`).setImage(u.displayAvatarURL({ size: 512 }))] }); });
addCmd(new SlashCommandBuilder().setName('userinfo').addUserOption(o => o.setName('utilisateur').setDescription('Utilisateur (optionnel)')), async i => { const u = i.options.getUser('utilisateur') || i.user; const m = await i.guild.members.fetch(u.id).catch(() => null); i.reply({ embeds: [baseEmbed().setTitle(`👤 ${u.username}`).addFields({name:'ID',value:u.id,inline:true},{name:'Bot ?',value:u.bot?'Oui':'Non',inline:true},{name:'Compte créé',value:`<t:${Math.floor(u.createdTimestamp/1000)}:R>`,inline:false},{name:'A rejoint',value:m?`<t:${Math.floor(m.joinedTimestamp/1000)}:R>`:'?',inline:false},{name:'Rôles',value:m&&m.roles.cache.size>1?m.roles.cache.filter(r=>r.id!==i.guild.id).map(r=>r.toString()).join(' '):'Aucun',inline:false}).setThumbnail(u.displayAvatarURL({ size: 256 }))] }); });
...
// --- MODÉRATION ---
addCmd(new SlashCommandBuilder().setName('warn').addUserOption(o => o.setName('utilisateur').setRequired(true)).addStringOption(o => o.setName('raison').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('warnings').addUserOption(o => o.setName('utilisateur').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('clearwarns').addUserOption(o => o.setName('utilisateur').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('kick').addUserOption(o => o.setName('utilisateur').setRequired(true)).addStringOption(o => o.setName('raison').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('ban').addUserOption(o => o.setName('utilisateur').setRequired(true)).addStringOption(o => o.setName('raison').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('unban').addStringOption(o => o.setName('id').setRequired(true)), async i => {
  if (!i.member.permissions.has(PermissionFlagsBits.BanMembers)) return i.reply({ content: '❌ Permission insuffisante.', ephemeral: true });
  await i.guild.members.unban(i.options.getString('id')); i.reply({ content: '✅ Utilisateur débanni.' });
});
addCmd(new SlashCommandBuilder().setName('timeout').addUserOption(o => o.setName('utilisateur').setRequired(true)).addStringOption(o => o.setName('duree').setRequired(true).setDescription('Ex: 10m, 1h, 1d')).addStringOption(o => o.setName('raison').setRequired(true)), async i => {
...
addCmd(new SlashCommandBuilder().setName('clear').addIntegerOption(o => o.setName('nombre').setRequired(true).setMinValue(1).setMaxValue(100)), async i => {
...
addCmd(new SlashCommandBuilder().setName('say').addStringOption(o => o.setName('message').setRequired(true)), async i => { if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true }); await i.channel.send(i.options.getString('message')); i.reply({ content: '✅ Envoyé.', ephemeral: true }); });
addCmd(new SlashCommandBuilder().setName('embed').addStringOption(o => o.setName('titre').setRequired(true)).addStringOption(o => o.setName('description').setRequired(true)).addStringOption(o => o.setName('couleur').setDescription('Hex sans # (ex: 0B5BD7)')), async i => {
  if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true });
...
addCmd(new SlashCommandBuilder().setName('annonce').addStringOption(o => o.setName('message').setRequired(true)), async i => { if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true }); await i.channel.send({ embeds: [baseEmbed().setTitle('📢 Annonce').setDescription(i.options.getString('message'))] }); i.reply({ content: '✅ Annonce publiée.', ephemeral: true }); });
addCmd(new SlashCommandBuilder().setName('giveaway').addStringOption(o => o.setName('duree').setRequired(true).setDescription('Ex: 10m, 1h, 1d')).addStringOption(o => o.setName('prix').setRequired(true)), async i => {
  if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true });
  const ms = parseDuration(i.options.getString('duree')); if (!ms) return i.reply({ content: 'Durée invalide.', ephemeral: true });
