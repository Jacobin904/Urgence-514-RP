...
addCmd(
  new SlashCommandBuilder().setName('help').setDescription('Liste de toutes les commandes'), 
  async (i) => {
    i.reply({ embeds: [baseEmbed().setTitle('📚 Aide — Urgence 514 RP').addFields(
...
addCmd(
  new SlashCommandBuilder().setName('info').setDescription('Informations sur le serveur'), 
  async (i) => {
    i.reply({ embeds: [baseEmbed().setTitle('🚨 Urgence 514 RP').setDescription('Serveur roleplay Roblox immersif basé sur l\'île de Montréal.').addFields(
...
addCmd(
  new SlashCommandBuilder().setName('code').setDescription('Affiche le code Roblox'), 
  async (i) => { 
    i.reply({ embeds: [baseEmbed().setTitle('🎮 Code Roblox').setDescription('Entre ce code dans Roblox :\n# `urgrp`')] }); 
  }
);
...
addCmd(
  ...
  async (i) => { 
    const t = Date.now(); 
    await i.reply('🏓 Ping...'); 
    i.editReply(`🏓 Pong ! Latence : **${Date.now() - t}ms** • API : **${Math.round(client.ws.ping)}ms**`); 
  }
);
...
addCmd(
  new SlashCommandBuilder()
    .setName('regles')
    .setDescription('Affiche les règlements')
    .addStringOption(option => 
      option.setName('type')
        .setDescription('Quel règlement ?')
        .setRequired(true)
        .addChoices(
...
addCmd(
  new SlashCommandBuilder()
    .setName('langage')
    .setDescription('Dictionnaire du langage RP')
    .addStringOption(option => 
...
addCmd(
  new SlashCommandBuilder().setName('departements').setDescription('Liste des départements'), 
  async (i) => { 
...
addCmd(
  new SlashCommandBuilder().setName('equipe').setDescription("L'équipe du serveur"), 
  async (i) => { 
    i.reply({ embeds: [baseEmbed().setTitle('👥 Équipe').addFields(
...
addCmd(
  new SlashCommandBuilder().setName('recrutement').setDescription('Infos recrutement staff'), 
  async (i) => { 
    i.reply({ 
      embeds: [baseEmbed().setTitle('⭐ Recrutement Staff').setDescription('**Conditions :**\n• Être sur PC\n• 14 ans et +\n• 7 jours sur le serveur\n• Moins de 10 sanctions\n\n⚠️ Demander des nouvelles = refus automatique.')], 
...
addCmd(
  new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName('utilisateur').setDescription('L\'utilisateur (optionnel)')), 
  async (i) => { 
...
addCmd(
  ...
  async (i) => { 
    ...
    i.reply({ embeds: [baseEmbed().setTitle(`👤 ${u.username}`).addFields(
...
addCmd(
  ...
  async (i) => { 
    const g = i.guild; 
    i.reply({ embeds: [baseEmbed().setTitle(`🏙️ ${g.name}`).addFields(
...
addCmd(
  ...
  async (i) => {
    ...
    await ghWrite(list);
    i.reply({ embeds: [baseEmbed().setTitle('⚠️ Avertissement').addFields(
      { name: 'Utilisateur', value: `<@${target.id}>`, inline: true },
      { name: 'Raison', value: reason, inline: true },
      { name: 'Total', value: `${list.warnings[target.id].length}`, inline: true }
    )] });
...
addCmd(
  new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName('utilisateur').setDescription('L\'utilisateur à vérifier').setRequired(true)), 
  async (i) => {
    const target = i.options.getUser('utilisateur'); 
    const { list } = await ghRead(); 
...
addCmd(
  new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName('utilisateur').setDescription('L\'utilisateur dont il faut effacer les avertissements').setRequired(true)), 
  async (i) => {
...
addCmd(
  new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName('utilisateur').setDescription('L\'utilisateur à mettre en sourdine').setRequired(true))
    .addStringOption(option => option.setName('duree').setDescription('Durée (ex: 10m, 1h, 1d)').setRequired(true))
    .addStringOption(option => option.setName('raison').setDescription('La raison du timeout').setRequired(true)), 
  async (i) => {
...
addCmd(
  new SlashCommandBuilder()
    .setName('slowmode')
    .setDescription('Active le mode lent')
    .addIntegerOption(option => option.setName('secondes').setDescription('Durée du slowmode en secondes (0-21600)').setRequired(true).setMinValue(0).setMaxValue(21600)), 
  async (i) => { 
    if (!isStaff(i)) return i.reply({ content: '❌ Permission insuffisante.', ephemeral: true }); 
...
addCmd(
  new SlashCommandBuilder().setName('roles').setDescription('Crée le panel des rôles de notification'), 
  async (i) => {
    ...
    await i.channel.send({ 
      embeds: [baseEmbed().setTitle('🔔 Notifications').setDescription('Clique sur les boutons pour activer/désactiver tes notifications.')], 
...
addCmd(
  new SlashCommandBuilder()
    .setName('say')
    .setDescription('Envoie un message simple')
...
addCmd(
  new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('description').setDescription('La description de l\'embed').setRequired(true))
    .addStringOption(option => option.setName('couleur').setDescription('Couleur en hex sans # (ex: 0B5BD7)')), 
  async (i) => {
    if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true });
    let color = COLOR; 
    ...
    await i.channel.send({ 
      embeds: [new EmbedBuilder().setColor(color).setTitle(i.options.getString('titre')).setDescription(i.options.getString('description')).setFooter({ text: 'Urgence 514 RP • Développé par Jacobin904', iconURL: LOGO }).setTimestamp()] 
    });
...
addCmd(
  new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('message').setDescription('Le message de l\'annonce').setRequired(true)), 
  async (i) => { 
    if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true }); 
...
addCmd(
  new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('duree').setDescription('Durée (ex: 10m, 1h, 1d)').setRequired(true))
    .addStringOption(option => option.setName('prix').setDescription('Le prix du giveaway').setRequired(true)), 
  async (i) => {
    if (!isStaff(i)) return i.reply({ content: '❌ Staff requis.', ephemeral: true });
    const ms = parseDuration(i.options.getString('duree')); 
    if (!ms) return i.reply({ content: 'Durée invalide.', ephemeral: true });
...
async function endGiveaway(id) {
  ...
  if (msg) await msg.edit({ 
    embeds: [baseEmbed().setTitle('🎉 GIVEAWAY TERMINÉ').setDescription(`**Prix :** ${gw.prize}\n**Gagnant :** <@${winner}>`)], 
    components: [] 
  }).catch(() => {});
}
    ...
    try {
      if (i.customId.startsWith('notif:')) {
        ...
        if (i.member.roles.cache.has(role.id)) { 
          ...
          await i.reply({ content: `🔕 Rôle **${name}** retiré.`, ephemeral: true }); 
        } else { 
          await i.member.roles.add(role); 
          await i.reply({ content: `🔔 Rôle **${name}** ajouté.`, ephemeral: true }); 
        }
