const {
  ...
  SlashCommandBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, ActivityType
} = require('discord.js');
...
function baseEmbed(){
  return new EmbedBuilder().setColor(COLOR).setFooter({text:'Urgence 514 RP • Depuis 2026', iconURL:LOGO}).setTimestamp();
}
...
async function ghWrite(data){
  ...
  const body = {message:'bot data', content:Buffer.from(JSON.stringify(data,null,2)).toString('base64'), branch:GITHUB_BRANCH};
  if(sha) body.sha = sha;
...
function addCmd(builder, run, opts={}){ commands.set(builder.name, {builder, run, ...opts}); }

// --- PUBLIC ---
addCmd(new SlashCommandBuilder().setName('help').setDescription('Liste de toutes les commandes du bot'), async i => {
  const e = baseEmbed().setTitle('📚 Aide — Urgence 514 RP')
    .addFields(
      {name:'🌐 Public', value:'`/help` `/info` `/code` `/ping` `/regles` `/langage` `/departements` `/equipe` `/recrutement` `/avatar` `/userinfo` `/serverinfo`', inline:false},
...
addCmd(new SlashCommandBuilder().setName('info').setDescription('Informations sur le serveur'), async i => {
  i.reply({embeds:[baseEmbed().setTitle('🚨 Urgence 514 RP')
    .setDescription('Serveur roleplay Roblox immersif basé sur l\'île de Montréal. Police, pompiers, paramédics, civils : trouve ta place dans la métropole.')
    .addFields(
...
addCmd(new SlashCommandBuilder().setName('code').setDescription('Affiche le code Roblox du serveur'), async i => {
  i.reply({embeds:[baseEmbed().setTitle('🎮 Code Roblox').setDescription('Entre ce code dans Roblox pour rejoindre la ville :\n# `urgrp`')]});
});

addCmd(new SlashCommandBuilder().setName('ping').setDescription('Latence du bot'), async i => {
  const t = Date.now();
  await i.reply({content:'🏓 Ping...'});
  i.editReply({content:`🏓 Pong ! Latence : **${Date.now()-t}ms** • API : **${Math.round(client.ws.ping)}ms**`});
});

addCmd(new SlashCommandBuilder().setName('regles')
  .addStringOption(o=>o.setName('type').setDescription('Quel règlement ?').setRequired(true).addChoices({name:'Discord',value:'discord'},{name:'Roblox',value:'roblox'})),
  async i => {
    const type = i.options.getString('type');
    const data = type==='discord'?RULES_DISCORD:RULES_ROBLOX;
    const e = baseEmbed().setTitle(type==='discord'?'📜 Règlement Discord':'🎮 Règlement Roblox');
    data.forEach(([t,d])=>e.addFields({name:t,value:d,inline:false}));
    i.reply({embeds:[e]});
  });

addCmd(new SlashCommandBuilder().setName('langage')
  .addStringOption(o=>o.setName('terme').setDescription('Terme à chercher (optionnel)'), false),
  async i => {
    const terme = i.options.getString('terme');
    if(terme){
      const found = LANGAGE.find(([t])=>t.toLowerCase().includes(terme.toLowerCase()));
      if(found) return i.reply({embeds:[baseEmbed().setTitle('🗣️ Langage RP').addFields({name:found[0],value:found[1],inline:false})]});
      return i.reply({content:'Terme introuvable. Utilise `/langage` sans option pour tout voir.', ephemeral:true});
    }
    const e = baseEmbed().setTitle('🗣️ Dictionnaire RP');
    LANGAGE.forEach(([t,d])=>e.addFields({name:t,value:d,inline:false}));
    i.reply({embeds:[e]});
  });

addCmd(new SlashCommandBuilder().setName('departements').setDescription('Liste des départements'), async i => {
  const e = baseEmbed().setTitle('🚔 Départements');
  DEPARTEMENTS.forEach(([n,d])=>e.addFields({name:n,value:d,inline:false}));
  i.reply({embeds:[e]});
});

addCmd(new SlashCommandBuilder().setName('equipe').setDescription("L'équipe du serveur"), async i => {
  i.reply({embeds:[baseEmbed().setTitle('👥 Équipe')
    .addFields(
      {name:'👑 Fondateur', value:'𝐌𝐗𝐋𝐂𝐇𝐎𝐂𝐎𝐋𝐀𝐓.𝐐𝐂 (@maxlechocolat.qc)', inline:false},
...
addCmd(new SlashCommandBuilder().setName('recrutement').setDescription('Infos recrutement staff'), async i => {
  const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel('Postuler').setStyle(ButtonStyle.Link).setURL('https://jacobin904.github.io/Urgence-514-RP/Recrutement/'));
  i.reply({embeds:[baseEmbed().setTitle('⭐ Recrutement Staff')
    .setDescription('**Conditions :**\n• Être sur PC\n• 14 ans et +\n• 7 jours sur le serveur\n• Moins de 10 sanctions\n\n⚠️ Demander des nouvelles = refus automatique.')], components:[row]});
});

addCmd(new SlashCommandBuilder().setName('avatar')
  .addUserOption(o=>o.setName('utilisateur').setDescription('Utilisateur (optionnel)')),
  async i => {
    const u = i.options.getUser('utilisateur') || i.user;
    i.reply({embeds:[baseEmbed().setTitle(`Avatar de ${u.username}`).setImage(u.displayAvatarURL({size:512}))]});
  });

addCmd(new SlashCommandBuilder().setName('userinfo')
  .addUserOption(o=>o.setName('utilisateur').setDescription('Utilisateur (optionnel)')),
  async i => {
    const u = i.options.getUser('utilisateur') || i.user;
    const m = await i.guild.members.fetch(u.id).catch(()=>null);
    i.reply({embeds:[baseEmbed().setTitle(`👤 ${u.username}`)
      .addFields(
        {name:'ID', value:u.id, inline:true},
        {name:'Bot ?', value:u.bot?'Oui':'Non', inline:true},
        {name:'Compte créé', value:`<t:${Math.floor(u.createdTimestamp/1000)}:R>`, inline:false},
        {name:'A rejoint', value:m?`<t:${Math.floor(m.joinedTimestamp/1000)}:R>`:'?', inline:false},
        {name:'Rôles', value:m&&m.roles.cache.size>1?m.roles.cache.filter(r=>r.id!==i.guild.id).map(r=>r).join(' '):'Aucun', inline:false}
      ).setThumbnail(u.displayAvatarURL({size:256}))]});
  });

addCmd(new SlashCommandBuilder().setName('serverinfo').setDescription('Infos du serveur'), async i => {
  const g = i.guild;
  i.reply({embeds:[baseEmbed().setTitle(`🏙️ ${g.name}`)
    .addFields(
      ...
      {name:'Créé', value:`<t:${Math.floor(g.createdTimestamp/1000)}:R>`, inline:false}
    ).setThumbnail(g.iconURL({size:512}))]});
});

// --- MODÉRATION ---
addCmd(new SlashCommandBuilder().setName('warn')
  ...
  async i => {
    ...
    list.warnings[target.id] = list.warnings[target.id] || [];
...
addCmd(new SlashCommandBuilder().setName('warnings')
  .addUserOption(o=>o.setName('utilisateur').setRequired(true)),
  async i => {
    const target = i.options.getUser('utilisateur');
    const {list} = await ghRead();
    const w = list.warnings[target.id] || [];
    if(!w.length) return i.reply({content:`✅ ${target.username} n'a aucun avertissement.`, ephemeral:true});
    const e = baseEmbed().setTitle(`⚠️ Avertissements de ${target.username}`);
    w.slice(-10).forEach((x,idx)=>e.addFields({name:`#${idx+1} — ${x.reason}`,value:`Par ${x.by}`,inline:false}));
    i.reply({embeds:[e]});
  });

addCmd(new SlashCommandBuilder().setName('clearwarns')
  .addUserOption(o=>o.setName('utilisateur').setRequired(true)),
  async i => {
    ...
    const target = i.options.getUser('utilisateur');
    const {list} = await ghRead();
    delete list.warnings[target.id];
    await ghWrite(list);
    i.reply({content:`✅ Avertissements de ${target.username} effacés.`});
  });

addCmd(new SlashCommandBuilder().setName('kick')
  .addUserOption(o=>o.setName('utilisateur').setRequired(true))
  .addStringOption(o=>o.setName('raison').setRequired(true)),
  async i => {
    ...
    i.reply({embeds:[baseEmbed().setTitle('👢 Kick').setDescription(`${m.user.username} a été kick. Raison : ${i.options.getString('raison')}`)]});
  });

addCmd(new SlashCommandBuilder().setName('ban')
  .addUserOption(o=>o.setName('utilisateur').setRequired(true))
  .addStringOption(o=>o.setName('raison').setRequired(true)),
  async i => {
...
addCmd(new SlashCommandBuilder().setName('unban')
  .addStringOption(o=>o.setName('id').setRequired(true)),
  async i => {
    if(!i.member.permissions.has(PermissionFlagsBits.BanMembers)) return i.reply({content:'❌ Permission insuffisante.', ephemeral:true});
    await i.guild.members.unban(i.options.getString('id'));
    i.reply({content:'✅ Utilisateur débanni.'});
  });

addCmd(new SlashCommandBuilder().setName('timeout')
  .addUserOption(o=>o.setName('utilisateur').setRequired(true))
  .addStringOption(o=>o.setName('duree').setRequired(true).setDescription('Ex: 10m, 1h, 1d'))
  .addStringOption(o=>o.setName('raison').setRequired(true)),
  async i => {
    ...
    if(!ms) return i.reply({content:'Durée invalide. Ex: `10m`, `1h`, `1d`.', ephemeral:true});
    const m = await i.guild.members.fetch(i.options.getUser('utilisateur').id).catch(()=>null);
    if(!m) return i.reply({content:'Membre introuvable.', ephemeral:true});
    await m.timeout(ms, i.options.getString('raison'));
    i.reply({embeds:[baseEmbed().setTitle('⏱️ Timeout').setDescription(`${m.user.username} mis en sourdine pendant ${i.options.getString('duree')}.`)]});
  });

addCmd(new SlashCommandBuilder().setName('clear')
  .addIntegerOption(o=>o.setName('nombre').setRequired(true).setMinValue(1).setMaxValue(100)),
  async i => {
...
addCmd(new SlashCommandBuilder().setName('lock').setDescription('Verrouille le salon'), async i => {
  if(!isStaff(i)) return i.reply({content:'❌ Permission insuffisante.', ephemeral:true});
  await i.channel.permissionOverwrites.edit(i.guild.id, {SendMessages:false});
  i.reply({content:'🔒 Salon verrouillé.'});
});

addCmd(new SlashCommandBuilder().setName('unlock').setDescription('Déverrouille le salon'), async i => {
  if(!isStaff(i)) return i.reply({content:'❌ Permission insuffisante.', ephemeral:true});
  await i.channel.permissionOverwrites.edit(i.guild.id, {SendMessages:true});
  i.reply({content:'🔓 Salon déverrouillé.'});
});

addCmd(new SlashCommandBuilder().setName('slowmode')
  .addIntegerOption(o=>o.setName('secondes').setRequired(true).setMinValue(0).setMaxValue(21600)),
  async i => {
    if(!isStaff(i)) return i.reply({content:'❌ Permission insuffisante.', ephemeral:true});
    await i.channel.setRateLimitPerUser(i.options.getInteger('secondes'));
    i.reply({content:`🐢 Slowmode : ${i.options.getInteger('secondes')}s.`});
  });
...
addCmd(new SlashCommandBuilder().setName('roles').setDescription('Crée le panel des rôles de notification'), async i => {
  ...
  const names = ['Spoiler','Nouveautés','Évènements & Giveaways','Live'];
  const emojis = ['👀','⭐','🎊','🔴'];
  const buttons = [];
  for(let x=0;x<names.length;x++){
    let role = i.guild.roles.cache.find(r=>r.name===names[x]);
    if(!role) role = await i.guild.roles.create({name:names[x], color:COLOR, reason:'Rôle notification'});
  ...
  await i.channel.send({embeds:[baseEmbed().setTitle('🔔 Notifications').setDescription('Clique sur les boutons pour activer/désactiver tes notifications.')], components:[row]});
  i.reply({content:'✅ Panel envoyé.', ephemeral:true});
});

addCmd(new SlashCommandBuilder().setName('say')
  .addStringOption(o=>o.setName('message').setRequired(true)),
  async i => {
    if(!isStaff(i)) return i.reply({content:'❌ Staff requis.', ephemeral:true});
    await i.channel.send(i.options.getString('message'));
    i.reply({content:'✅ Envoyé.', ephemeral:true});
  });

addCmd(new SlashCommandBuilder().setName('embed')
  .addStringOption(o=>o.setName('titre').setRequired(true))
  .addStringOption(o=>o.setName('description').setRequired(true))
  .addStringOption(o=>o.setName('couleur').setDescription('Hex sans # (ex: 0B5BD7)')),
  async i => {
    if(!isStaff(i)) return i.reply({content:'❌ Staff requis.', ephemeral:true});
    let color = COLOR;
    const hex = i.options.getString('couleur');
    if(hex) color = parseInt(hex,16) || COLOR;
    await i.channel.send({embeds:[new EmbedBuilder().setColor(color).setTitle(i.options.getString('titre')).setDescription(i.options.getString('description')).setFooter({text:'Urgence 514 RP',iconURL:LOGO}).setTimestamp()]});
    i.reply({content:'✅ Embed envoyé.', ephemeral:true});
  });

addCmd(new SlashCommandBuilder().setName('annonce')
  .addStringOption(o=>o.setName('message').setRequired(true)),
  async i => {
    if(!isStaff(i)) return i.reply({content:'❌ Staff requis.', ephemeral:true});
    await i.channel.send({embeds:[baseEmbed().setTitle('📢 Annonce').setDescription(i.options.getString('message'))]});
    i.reply({content:'✅ Annonce publiée.', ephemeral:true});
  });

addCmd(new SlashCommandBuilder().setName('giveaway')
  .addStringOption(o=>o.setName('duree').setRequired(true).setDescription('Ex: 10m, 1h, 1d'))
  .addStringOption(o=>o.setName('prix').setRequired(true)),
  async i => {
    if(!isStaff(i)) return i.reply({content:'❌ Staff requis.', ephemeral:true});
    const ms = parseDuration(i.options.getString('duree'));
    if(!ms) return i.reply({content:'Durée invalide.', ephemeral:true});
    ...
    i.reply({content:'✅ Giveaway lancé.', ephemeral:true});
  });

async function endGiveaway(id){
  const gw = giveaways.get(id); if(!gw) return;
  giveaways.delete(id);
  ...
  if(i.isButton()){
    try{
      if(i.customId.startsWith('notif:')){
        const name = i.customId.slice(6);
        let role = i.guild.roles.cache.find(r=>r.name===name);
        ...
        const gw = giveaways.get(i.customId.slice(3));
...
if(BOT_TOKEN) client.login(BOT_TOKEN).catch(e=>console.error('Bot login error:', e.message));
