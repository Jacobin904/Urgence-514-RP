...
// Intents nécessaires pour que le bot fonctionne correctement
...
// ===== ANTI-CRASH & KEEP ALIVE (Empêche Render de dormir) =====
...
async function ghRead() {
  try {
    const r = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/contents/botdata.json?ref=${GITHUB_BRANCH}`, {
      headers: { Authorization: `token ${GITHUB_TOKEN}`, 'User-Agent': 'urgence-514-bot' }
    });
...
async function ghWrite(data) {
  ...
  if (sha) body.sha = sha;
  await fetch(`https://api.github.com/repos/${GITHUB_REPO}/contents/botdata.json`, {
    method: 'PUT',
    headers: { Authorization: `token ${GITHUB_TOKEN}`, 'User-Agent': 'urgence-514-bot', 'Content-Type': 'application/json' },
...
if (BOT_TOKEN) {
  ...
  console.warn('⚠️ BOT_TOKEN manquant dans les variables d\'environnement Render.');
}
